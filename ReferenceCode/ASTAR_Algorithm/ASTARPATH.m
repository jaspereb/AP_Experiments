...
for k=1:size(GoalRegister,1)
    for j=1:size(GoalRegister,2)
        if MAP(k,j)==0 %Ignore obstacle cells
            Mat=RegisteredGoals-(repmat([j k],(Nodesfound),1));
            Distance=(min(sqrt(sum(abs(Mat).^2,2))));
            Hn(k,j)=Distance; %Manhattan distance from that cell to the nearest goal
        end
    end
end
...
while 1==1 %Code will break when path found or when no path exist
    ...
    for p=1:N_Neighboors
        ...
        if CurrentY+i<1||CurrentY+i>Height||CurrentX+j<1||CurrentX+j>Width
            continue %Stay on map
        end
        Flag=1;
        if(ClosedMAT(CurrentY+i,CurrentX+j)==0) %Neiboor is open;
            %Obstacle cells are already closed, so only need to check if
            %moving more than 1 space...
            if (abs(i)>1||abs(j)>1)
            ...
            if Flag==1 %Neighbour is reachable
                % i j k is the local offset, so this is cost to arrive + transition cost
